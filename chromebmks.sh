#!/bin/bash
declare -r SRC="$HOME/Documents/information/chromebookmarks/"
declare -r SRC_DEFAULT="$HOME/Documents/information/chromebookmarks/.csv"
...
synopsis() {
    clear
    local count
    local chromebmkshome="$(which chromebmks)"
    count="$(find ~/Documents/information/chromebookmarks/ -type f -iname "*.html" | sed -n "$=")"
    printf "\n%s%s\n\n" "$(color -x 179 "${0}")" "$(color -w " lives at $chromebmkshome")"
    printf "%s\n\t%s\n\t%s\n\t%s\n\n" \
...
while getopts ':?lc:o:' OPTION; do
    ...
    l)
        ...
        printf "\nThe HTML files live at: %s\n" "$SRC"
        ls -sh "$HOME/Documents/information/chromebookmarks/"
        cleanExit
        ;;
